...
class UpdateVoterRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $voterId = $voter instanceof User ? $voter->id : $voter;
        ...
        return [
            ...
            'email' => [
                ...
                'email',
                'max:255',
                Rule::unique('users', 'email')->ignore($voterId)->whereNull('deleted_at'),
            ],
            ...
            'nik' => [
                ...
                'size:16',
                'regex:/^\d{16}$/',
                Rule::unique('users', 'nik')->ignore($voterId)->whereNull('deleted_at'),
            ],
            ...
            // Password is optional on update; only validated if provided or login method changed
            'password' => [
                $needsPassword && $this->filled('password') ? 'sometimes' : 'nullable',
                'string',
                'min:8',
                'confirmed',
            ],
